...
export const tmdb = {
  ...
  getPopular: async (type: 'movie' | 'tv') => {
    const response = await fetchTMDB<TMDBResponse<Movie | TVShow>>(`/${type}/popular`);
    // Add media_type to each item since popular endpoint doesn't include it
    return {
      ...response,
  ...
  getTopRated: async (type: 'movie' | 'tv') => {
    const response = await fetchTMDB<TMDBResponse<Movie | TVShow>>(`/${type}/top_rated`);
    // Add media_type to each item since top_rated endpoint doesn't include it
    return {
      ...response,
      results: response.results.map(item => ({
        ...item,
